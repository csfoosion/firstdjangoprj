...
#post_new 는 폼으로 연결해주는 함수이다
def post_new(request):
    #7.9
    #request는 사용자의 요청, method는 요청 방식
    #GET방식으로 들어온 요청인지 POST방식으로 들어온 요청인지 검사, 
    #form 의 method="post"이므로 form 을 사용해 들어온 자료는 무조건 post 여야함
    if request.method == "POST":
        form = PostForm(request.POST)
        #.is_valid()는 form 을 통해 넘어온 자료가 유효한 자료인지 검증
        if form.is_valid():
        ...
        #현재 PostForm() 양식을 따라 만들것임을 보여준다
        form = PostForm()
    #저장한 폼 양식은 텀플릿 파일에 같이 넘겨야 한다
    return render(request, 'blog/post_edit.html', {'form':form})


def post_edit(request, pk):
    #사용자 에러 페이지 
    #글을 수정해야 하기 때문에 기종에 입력되어 있던 자료를 가져오는게 먼저임
    post = get_object_or_404(Post, pk = pk)
    #get방식 post방식 구분
    if request.method == "POST":
        ...
        #만약 post 방식이 아닌 경우라면 get 방식이므로 수정 직전임
        #따라서 폼으로 다시 연결해줘야함, 이때의 촘은 수정용 폼이며
        #수정용 폼에는 기존에 써놨었던 글이 먼저 입력되어 있어야 하므로
        #감안해서 기존 글의 냐용이 담겨있는 post를 폼에 instance로 넘겨줌
